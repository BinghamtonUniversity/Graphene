GrapheneDataGrid = function (options) {
  ...
  //not sure why this was added - cjm
  //this.showSearch = !!options.search;
  var actions = [
    {
      type: "danger",
      name: "delete",
  ...
  options.filterFields = _.map(_.extend({}, options.schema), function (val) {
    ...
    delete val.size;
    delete val.pre;
    // not sure why these were added cjm
    // delete val.horizontal;
    // delete val.columns;
    // delete val.offset;
    delete val.opst;
    // val.isEnabled = true;
...
/*added one label and fixed others for accessibility*/
gform.stencils[
  "filter"
] = `<div name="{{name}}" id="{{id}}" data-type="{{type}}" style="position:relative">
{{^hideLabel}}<label for="filter_{{id}}">{{{label}}}{{^label}}&nbsp;{{/label}}</label>{{/hideLabel}}
<span class="badge count" style="position: absolute;top: -5px;left: -5px;background:#466769"></span>
<button type="button" id="filter_{{id}}" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="width: 100%;display: flex;justify-content: space-between;align-items: center;padding-left: 1em;padding-right: 0.5em;gap: 0.5em;">
  Filter <span class="caret"></span>
</button>
<ul class="dropdown-menu" style="left:initial;padding: 5px 0 0;z-index: 10;">

  <li style="position:relative">
  <label class='sr-only'>Search Text
    <input type="text" placeholder="Search..." aria-label='Search' style="margin:-3px 2px 2px;width:auto" class="form-control" aria-label="search text" /> <div class="gclear" >
  </label>
...
<div class="">
  ...
  <label for="min_{{id}}" class="sr-only">Minimum {{name}}</label>
  ...
  <label for="max_{{id}}" class="sr-only">Maximum {{name}}</label>
...
<table style="width:100%">
  <thead>
    <tr>
      ...
      {{^name}}
        <td>
          <div class="btn-group pull-left" style="white-space: nowrap; font-size: 0;" role="group" aria-label="..."></div>
        </td>
          <td style="width:100%">
            <div class="btn-group pull-{{^align}}left{{/align}}{{align}}" style="margin-left:15px;white-space: nowrap; font-size: 0;" role="group" aria-label="...">
          </td>
...
/*added label, aria-labels, tab-indexes for accessibility*/
...
{{/options.filter}}
	...
	<div class="col-xs-6">
			{{#options.search}}<input type="text" name="search" aria-label="Search the data" class="form-control" style="" placeholder="Search">{{/options.search}}
					</div>
	</div>
	<div class="input-group">
			<span class="" style="display: table-cell;width: 1%;white-space: nowrap;vertical-align: middle;padding-right:5px">
					<button class="btn btn-default reverse" type="button" tabindex="-1"><i class="fa fa-sort text-muted" tabindex="0"></i></button>
			</span>
...
<div data-event="mark" data-id="{{[[}}id{{]]}}" tabindex="0" style="text-align:left;padding:0;-webkit-touch-callout: none;-webkit-user-select: none;-khtml-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;">
...
<div style="height:40px;">
  ...
  <label for="enables_{{options.id}}" class='sr-only'>Select columns to show</label>
  <button class="btn btn-default dropdown-toggle" type="button" aria-label="Select columns to show" id="enables_{{options.id}}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
      Actions
...
{{/options.actions}}
	...
	<input type="file" aria-label="CSV file for input" class="csvFileInput" accept=".csv" style="display:none">
  ...
  <div class="btn-group pull-left" style="white-space: nowrap; font-size: 0;" role="group" aria-label="...">
	...
	<div class="btn-group pull-right" style="margin-bottom:10px" role="group" aria-label="...">
      ...
      <!-- added tabindex for accessibility -->
			<div class="btn btn-default hidden-xs" name="bt-download" data-toggle="tooltip" data-placement="left" title="Download" tabindex="0"><i class="fa fa-download"></i></div>
			{{/options.download}}
			{{#options.upload}}
      <!-- added tabindex for accessibility -->
			<div class="btn btn-default hidden-xs" name="bt-upload" data-toggle="tooltip" data-placement="left" title="Upload" tabindex="0"><i class="fa fa-upload"></i></div>
			{{/options.upload}}


			{{#options.columns}}
			<div class="btn-group columnEnables" data-toggle="tooltip" data-placement="left" title="Display Columns">
      <!-- added labels and contectual text for accessibility -->
          <label for="enables_{{options.id}}" class='sr-only'>Select columns to show</label>
					<button class="btn btn-default dropdown-toggle" type="button" id="enables_{{options.id}}" aria-label='Select columns to show' data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
							<i class="fa fa-list"></i>
							<span class="caret"><span class='sr-only'>Select Columns</span></span>
...
{{^options.hideCheck}}
<div style="padding: 16px 0 0 15px;">
<!-- added label for accessibility -->
<label for="select_all_1" class="sr-only">Select All</label>
<i id="select_all_1" name="select_all" class="fa fa-2x fa-square-o" aria-label="Select All" tabindex="0"></i></div>
{{/options.hideCheck}}
...
// added rand to the end of ids to make sure element ids
// are unique
function rand() {
...
<!-- added aria-label for accessibility -->
<input type="file" class="csvFileInput" aria-label="CSV file for input" accept=".csv" style="display:none">
<div class="hiddenForm" style="display:none"></div>
...
<div class="btn-group pull-right" style="margin-bottom:10px;flex-shrink: 0;" role="group" aria-label="...">
  ...
  <!-- added label for accessibility -->
    <label for="enables_{{options.id}}" class='sr-only'>Select columns to show</label>
          ...
          <!-- added label for accessibility -->
          {{#label}}
					<li><label for="checkbox_{{id}}" data-field="{{id}}" style="font-weight:normal;display: flex;align-items: flex-start;gap: 5px;user-select: none;"><input id="checkbox_{{id}}" type="checkbox" {{#isEnabled}}checked="checked"{{/isEnabled}}> {{label}}</label></li>
					{{/label}}
          {{/visible}}
					{{/items}}
			</ul>
...
{{^options.query}}
{{#options.search}}
    <!-- added label, aria-label for accessibility -->
    <label class="sr-only">Search
    <input type="text" autocomplete="off" name="search" class="form-control pull-right" style="max-width:300px; 
    margin-bottom:10px" aria-label="query search" placeholder="Search" id="grapheneOptsSearch` + rand() + `">{{/options.search}}
    </label>
{{/options.query}}
...
/*added label for accessibility*/
gform.stencils.data_grid += `<label class="sr-only">Search</span>
<input type="text" autocomplete="off" name="query" class="form-control pull-left" style=" aria-label="query bar" 
margin-bottom:10px;max-width:{{options.query.width}}%;" placeholder="" id="grapheneOptionsSearchBar` + rand() + `"></label>
{{/options.query.bar}}
</div>
...
<!-- added caption for accessibility -->
<caption class="sr-only" style="background-color:#000; color:#fff">Graphene Datagrid Query Result</caption>
<thead class="head">
{{>data_grid_head}}
</thead>
</table>
...
{{#options.autoSize}}
	<thead>
        ...
        <!-- added screen reader text for accessibility -->
          <th style="width:60px" class="select-column" aria-label="Select All"><span class="sr-only">Select All</span></th>
        {{/options.hideCheck}}
				{{#items}}
          {{#visible}}
          <!-- added screen reader text for accessibility -->
            <th style="min-width:85px"><span class="sr-only">Filter By Column Value</span></th>
          {{/visible}}
          {{^visible}}
          <!-- added screen reader text for accessibility -->
            <th style="min-width:85px"><span class="sr-only">Hidden Unused Table Cell</span></th>
          {{/visible}}
        {{/items}}
    </tr>
  </thead>
...
<h5 class="range badge {{^size}}alert-danger{{/size}} pull-left" style="margin-right:15px; color: #000">{{#size}}Showing {{first}} to {{last}} of {{size}} results{{/size}}{{^size}}No matching results{{/size}}</h5>
{{#entries.length}}
<span class="pull-left">
	<select class="form-control" style="display:inline-block;width:auto;min-width:50px" name="count" aria-label="results count" >
...
{{^options.hideCheck}}
<th style="width: 60px;min-width:60px;padding: 0 0 0 20px;" class="select-column">
<!-- added label for accessibility -->
<label for="select_all_1" class="sr-only">Select All</label>
<i name="select_all" id="select_all_1" class="fa fa-2x fa-square-o" aria-label="Select All" tabindex="0"></i></th>
{{/options.hideCheck}}
...
<!-- added label for accessibility -->
{{#label}}
<th {{#options.sort}}data-sort="{{cname}}"{{/options.sort}}><h6 style="margin: 2px;font-size:13px;white-space: nowrap">{{#options.sort}}<i class="fa fa-sort text-muted" tabindex="0"></i> {{/options.sort}}{{{label}}}</h6></th>
{{/label}}
{{^label}}
<td {{#options.sort}}data-sort="{{cname}}"{{/options.sort}}></td>
{{/label}}
{{/visible}}
{{/items}}
</tr>
...
<!-- added tab stop for accessibility -->
<td data-event="mark" data-id="{{[[}}id{{]]}}" tabindex="0" style="width: 60px;min-width:60px;text-align:left;padding:0;-webkit-touch-callout: none;-webkit-user-select: none;-khtml-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;">
