...
{{#options.query.bar}}
<input type="text" autocomplete="off" name="query" class="form-control pull-left" style=" aria-label="query bar" margin-bottom:10px;max-width:{{options.query.width}}%;" placeholder="">
{{/options.query.bar}}
</div>
...
{{/multiPage}}	
<h5 class="range badge {{^size}}alert-danger{{/size}} pull-left" style="margin-right:15px; background-color:#E6E6E6">{{#size}}Showing {{first}} to {{last}} of {{size}} results{{/size}}{{^size}}No matching results{{/size}}</h5>
{{#entries.length}}
