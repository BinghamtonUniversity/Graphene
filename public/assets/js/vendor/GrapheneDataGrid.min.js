...
{{^options.query}}
{{#options.search}}
    <label for="grapheneOptsSearch` + rand + `" class="sr-only">Search</label>
    <input type="text" autocomplete="off" name="search" class="form-control pull-right" style="max-width:300px; margin-bottom:10px" aria-label="query search" placeholder="Search" id="grapheneOptsSearch` + rand + `">{{/options.search}}
{{/options.query}}
<span style="margin-right:auto" name="count"></span>

{{#options.query.bar}}`;

let randOpt = Math.floor(Math.random() * (100000 + 1))
gform.stencils.data_grid += `<label for="grapheneOptionsSearchBar` + randOpt + `" class="sr-only">Search</span>
<input type="text" autocomplete="off" name="query" class="form-control pull-left" style=" aria-label="query bar" margin-bottom:10px;max-width:{{options.query.width}}%;" placeholder="" id="grapheneOptionsSearchBar` + randOpt + `">
{{/options.query.bar}}
</div>
