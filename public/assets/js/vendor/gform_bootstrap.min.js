...
gform.stencils = {
  ...
  switch: `<div class="row clearfix {{modifiers}} {{#array}}isArray" data-min="{{multiple.min}}" data-max="{{multiple.max}}{{/array}}" data-type="{{type}}">
      ...
      <span class="falseLabel">{{{options.0.label}}} </span>
          <span class="switch">
          <!-- if switches aren't working, look here - cjm -->
          {{#name}}
            {{^label}}
              <label for={{id}} class="sr-only">{{name}}</label>
            {{/label}}
  ...
  textarea: `<div class="row clearfix form-group {{modifiers}} {{#array}}isArray" data-min="{{array.min}}" data-max="{{array.max}}{{/array}}" data-type="{{type}}">
      ...
      {{#hideLabel}}
        {{#name}}
          <label for="{{id}}" class="sr-only">{{name}} Text Entry</label>
        {{/name}}
        {{^name}}
  ...
  _label: `
      {{^hideLabel}}
      {{#label}}
        <label for="{{id}}" {{^horizontal}}style="text-align:left"{{/horizontal}} class="control-label 
            {{^horizontal}}col-md-12{{/horizontal}}
            {{#horizontal}}col-md-4{{/horizontal}}">{{{label}}}{{#required}}{{{requiredText}}}{{/required}}{{suffix}}</label>{{/label}}
