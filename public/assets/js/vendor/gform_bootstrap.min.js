var gform = function (optionsIn, el) {
  ...
  this.getPath =
    typeof this.options.getPath == "function"
      ? this.options.getPath
      : function (options) {
          if (typeof options == "string") {
            options = { path: options };
          }
          let { path = "", search = "", ...rest } = options;
          return path[0] !== "/"
            ? gform.renderString(this.options.rootpath || "{{path}}", {
                ...this.get(),
                ...options,
                search,
                path,
              })
            : gform.renderString(path, {
                ...this.get(),
                ...rest,
              });
        };
  ...
  if (typeof this.options.data == "string") {
    if (typeof window.location[this.options.data] !== "undefined") {
      ...
      // }
      let path = this.owner.getPath({ path: this.options.data });
      gform.ajax({
        path: path,
...
gform.ajax = function (options) {
  ...
  if (
    ...
    if (typeof token !== "undefined" && "content" in token.attributes) {
...
gform.prototype.options = {
  ...
  default: {
    type: "text",
    suffix: ":",
    requiredText: '<span style="color:#B30000">*</span>',
    validate: [],
    valid: true,
    parsable: true,
    reportable: true,
  ...
  if (_.isString(this.optgroup.path) && this.optgroup.path) {
    if (
      ...
      if (this.collections.get(this.optgroup.path) !== "waiting") {
        ...
        let path = this.owner.getPath({ path: this.optgroup.path });
        gform.ajax({
          path: path,
          success: function (data) {
...
gform.processConditions = function (conditions, func, options) {
  ...
  if (typeof conditions === "object") {
    var callback = function (rules, func, _lookup, e) {
      func.call(this, gform._rules.call(this, rules, _lookup, null, e), e);
...
gform._rules = function (rules, _lookup, op, e) {
  var op = op || "and";
  return _.reduce(
    rules,
    function (result, rule) {
      ...
      if (typeof rule.conditions !== "undefined") {
        s = gform._rules.call(this, rule.conditions, _lookup, rule.op, e);
      } else {
        s = gform.conditions[rule.type](this, rule, _lookup, e);
      }
      if (op == "or") {
        return result || s;
      } else {
...
gform.conditions = {
  ...
  test: (field, args, _lookup, e) => args.test.call(this, field, args, e),
  method: (field, args, _lookup, e) => {
    if (
      args.value in field.owner.methods &&
      typeof field.owner.methods[args.value] == "function"
    ) {
      e.ref = _lookup(field, args);
      e.field = field;
      return !!field.owner.methods[args.value].call(null, e);
    }
    return false;
  },
...
gform.stencils = {
  ...
  // added sr-only label when hideLabel is true
  ...
  text: `<div class="row clearfix form-group {{modifiers}}" data-type="{{type}}">
      ...
      {{#hideLabel}}
        {{#name}}
          <label for="{{id}}" class="sr-only">Filter {{name}}</label>
        {{/name}}
      {{/hideLabel}}
  ...
  // added a label
    switch: `<div class="row clearfix {{modifiers}} {{#array}}isArray" data-min="{{multiple.min}}" data-max="{{multiple.max}}{{/array}}" data-type="{{type}}">
      ...
      <span class="falseLabel">{{{options.0.label}}} </span>
          <label class="switch"><span class='sr-only'>{{name}} slider</span>
  ...
  textarea: `<div class="row clearfix form-group {{modifiers}} {{#array}}isArray" data-min="{{array.min}}" data-max="{{array.max}}{{/array}}" data-type="{{type}}">
      ...
      {{/label}}
      {{^name}}
        <label for="{{id}}" class="sr-only">Text Entry</label>
      {{/name}}
  ...
  select: `<div class="row clearfix form-group {{modifiers}} {{#size}}size={{size}}{{/size}} {{#array}}isArray" data-min="{{array.min}}" data-max="{{array.max}}{{/array}}" data-type="{{type}}">
      ...
      {{#label}}
        ...
        {{#hideLabel}}
          {{#name}}
            <label for="{{id}}" class="sr-only">Select {{name}}</label>
          {{/name}}
        {{/hideLabel}}
      {{/label}}
      {{^label}}
  ...
  _label: `
      {{^hideLabel}}
      {{#label}}
      <label for="{{id}}" {{^horizontal}}style="text-align:left"{{/horizontal}} class="control-label {{^horizontal}}col-md-12{{/horizontal}}{{#horizontal}}col-md-4{{/horizontal}}">{{{label}}}{{#required}}{{{requiredText}}}{{/required}}{{suffix}}</label>
      {{/label}}
  ...
  checkbox: `<div class="row clearfix {{modifiers}} {{#array}}isArray" data-min="{{multiple.min}}" data-max="{{multiple.max}}{{/array}}" data-type="{{type}}">
      ...
      <div class="col-md-12" style="margin: -10px 0 5px;"">
        <label for="{{id}}" class="sr-only">Option {{{options.1.label}}}</label>
      {{/label}}
          <div class="checkbox">
               <span class="checkbox-holder" style="padding-left: 20px;">
  ...
  //added some context for accessibility
      ...
      <div class="modal-dialog">
          <div class="modal-content">
              <div class="modal-header {{modal.header_class}}">
                  ...
                  <h4 class="modal-title" id="myModalLabel">{{#icon}}<i class="fa fa-{{icon}}"></i>{{/icon}}
                          {{#title}}
                            {{{title}}}{{{legend}}}&nbsp;
                          {{/title}}
                          {{^title}}
                             ...
                             {{/legend}}
                             {{^legend}}
                              <div class="sr-only">Model popup for {{name}}&nbsp;</div>
                             {{/legend}}
                          {{/title}}
                  </h4>
              </div>
...
if (typeof $.fn !== "undefined" && "datetimepicker" in $.fn) {
