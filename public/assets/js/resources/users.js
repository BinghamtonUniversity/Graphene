...
user_form_attributes = [
  { name: "id", label: "Graphene ID", edit: false },
  {
    name: "unique_id",
    label: "Unique ID",
    edit: [{ type: "matches", name: "modify_unique_id", value: true }],
    parse: "edit",
  },
  ...
  {
    type: "switch",
    label: "Modify Unique ID",
    name: "modify_unique_id",
    value: false,
  ...
  fields: [
    {
      ...
      help: "Start typing a name to search i.e. john",
    },
    {
      type: "output",
      name: "results",
      label: false,
      format: { value: templates["user_list"].template },
    },
  ],
})
    ...
    success: function (data) {
      ...
      new gform({
        actions: [
          ...
          {
            type: "button",
            label: '<i class="fa fa-user"></i> Impersonate User',
            action: "impersonate",
