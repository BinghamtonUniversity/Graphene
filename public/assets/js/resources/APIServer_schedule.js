  ...
  success: function (data) {
    ...
    data = _.map(data, function (attributes) {
      var duration = moment.duration(
        moment(attributes.last_exec_start).diff(
          moment(attributes.last_exec_stop)
        )
      );
      attributes.execution_time = {
        scheduled: moment(attributes.last_exec_cron).fromNow(),
    ...
    tableConfig.schema = [
      ...
      },
      {
        ...
        required: true,
        type: "select",
        options: "/api/proxy/" + slug + "/api_instances",
        format: {
      ...
      { label: "Route", name: "route", required: true },
      {
        ...
        type: "checkbox",
        template:
          "{{#attributes.enabled}}Yes{{/attributes.enabled}}{{^attributes.enabled}}No{{/attributes.enabled}}",
        options: [
      ...
      },
      {
        ...
        label: "Arguments",
        template:
          "{{#attributes.args}}{{name}}=>{{value}}<br>{{/attributes.args}}",
      },
      {
        show: false,
        name: "times",
        label: "Status",
        template:
          "Last executed: {{attributes.execution_time.scheduled}}<br>Ran for {{attributes.execution_time.minutes}} minutes and {{attributes.execution_time.seconds}} seconds",
      },
      { name: "id", type: "hidden" },
    ];
    tableConfig.actions = [
    ...
    grid
      .on("model:params", function (grid_event) {
        new gform({
          name: "param_form",
          legend: "Args",
          fields: [
          ...
          .on("save", function (form_event) {
            ...
            $.ajax({
              url: url + "/" + grid_event.model.attributes.id,
              type: "PUT",
      ...
      .on("model:run", function (grid_event) {
        if (
          confirm(
            "Are you sure you want to run this Scheduled Job?  This action cannot be undone!"
          )
        ) {
          $.ajax({
            url: url + "/" + grid_event.model.attributes.id + "/run",
            type: "GET",
