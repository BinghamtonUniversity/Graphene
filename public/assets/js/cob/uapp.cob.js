Cobler.types.uApp = function (container) {
  ...
  var fields = {
    ...
    "App ID": {
      type: "select",
      options: "/api/groups/" + group_id + "/appinstances",
      format: {
        label: "{{name}}",
        value: function (i) {
          return i.id;
  ...
  return {
    ...
    initialize: function (el) {
      if (typeof this.get().app_id == "undefined") {
        return false;
      }
      ...
      $.ajax({
        ...
        type: "POST",
        data: Lockr.get(
          "/api/apps/instances/" + this.get().app_id + "/user_options"
        ) || { options: {} },
        success: function (data) {
          ...
          opts.config =
            (
              _.find(apps, { id: parseInt(this.get().app_id, 10) }) ||
              _.find(
                gform.collections.get(
                  "/api/groups/" + group_id + "/appinstances"
                ),
                { id: parseInt(this.get().app_id, 10) }
              )
            ).app.code || {};
          ...
          switch (opts.config.engine) {
            case "graphene":
              if ("graphene" in $g.engines) {
                this.appEngine = $g.engines[opts.config.version || "v1"]["v1"](
                  opts,
                  item
                );
              } else {
                this.appEngine = gAE_v0001(opts, item);
              }
              break;
            case "vue":
              this.appEngine = vueAppEngine(opts, item);
              break;
            default:
              // this.appEngine = vue_v0001(opts);
              // this.appEngine = grapheneAppEngine(opts);
              this.appEngine = $g.engines["graphene"]["v1"](opts, item);
          }
