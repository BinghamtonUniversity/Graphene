Cobler.types.WorkflowSubmissionReport = function (container) {
  ...
  return {
    ...
    fields: fields,
    render: function () {
      return workflow_report.container;
    },
    ...
    processFile: function (file) {
      ...
      if (file.deleted_at) {
        file.icon =
          '<i class="fa fa-image fa-3x" style="padding-top: 4px;"></i>';
      }
      file = $g.formatFile(file);
      ...
      if (file.ext == "pdf") {
        file.preview =
          ...
          ($(document).height() - $(".report").position().top - 100) +
          'px" src="' +
          file.path +
          '"></iframe>';
      }
    ...
    initialize: function (el) {
      $("body").append(
        "<style>@media (min-width: 992px) {.modal.gform .modal-dialog{width:992px}}</style>"
      );

      if (this.container.owner.options.disabled && this.get().enable_min) {
        var collapsed = (
          Lockr.get(this.get().guid) || { collapsed: this.get().collapsed }
        ).collapsed;
      ...
      $.ajax({
        url:
          "/workflows/fetch/" +
          this.get().options.workflow_instance_id +
          "/all/" +
          this.get().options.id,
        dataType: "json",
        type: "GET",
        success: function (resources) {
          $.ajax({
            url:
              "/api/workflowsubmissions/" + this.get().options.id + "/history",
            dataType: "json",
            type: "GET",
            success: function (resources, response) {
              ...
              data = _.orderBy(
                ...
                ).concat(_.map(response.files, this.processFile.bind(this))),
                "date",
                "desc"
              );
              ...
              templates = _.merge(
                ...
                _.mapValues(
                  _.keyBy(
                    _.filter(
                      this.get().options.workflow_version.code.templates,
                      function (e) {
              ...
              mappedData.owner.is = {
                actor: mappedData.owner.unique_id == mappedData.actor.unique_id,
              };
              mappedData.actor.is = {
                owner: mappedData.owner.unique_id == mappedData.actor.unique_id,
              };
              mappedData.assignment = {
                type: this.get().options.assignment_type,
                id: this.get().options.assignment_id,
              };
              if (mappedData.assignment.type == "user") {
                mappedData.assignment.user = _.pick(
                  this.get().assignment,
                  "first_name",
                  "last_name",
                  "email",
              ...
              mappedData.workflow = {
                name: this.get().options.workflow.name,
                description: this.get().options.workflow.description,
                instance: this.get().options.workflow_instance,
              };
              mappedData.current_state =
                _.find(mappedData.workflow.instance.version.code.flow, {
                  name: mappedData.state,
                }) || mappedData.workflow.instance.version.code.flow[0];
              ...
              mappedData.is = {
                ...
                initial:
                  mappedData.state ==
                  mappedData.workflow.instance.configuration.initial,
                // actionable:(this.get().is_assigned && mappedData.actions.length)
              };

              if (data.length > 1) {
                mappedData.previous = _.pick(data[1], "state", "status");
                mappedData.previous.state =
                  mappedData.previous.end_state ||
                  mappedData.workflow.instance.configuration.initial;
                mappedData.was = {
              ...
              mappedData.history = _.map(data, function (event) {
                ...
                newEvent.actor = _.pick(
                  event.deleted_by || event.user,
                  "first_name",
                  "last_name",
                  "email",
                  "unique_id",
                ...
                newEvent.actor.is = {
                  owner: mappedData.owner.unique_id == newEvent.actor.unique_id,
                };
              ...
              mappedData.original = _.findLast(
                data,
                ({ log, status }) => log == true && status == "open"
              );

              mappedData.latest = _.find(data, { log: true });
              mappedData.datamap = {};
              _.each(
                mappedData.workflow.instance.configuration.map,
                function (item) {
              ...
              mappedData.actions = _.filter(
                (
                  _.find(this.get().options.workflow_version.code.flow, {
                    name: this.get().options.state,
                  }) || { actions: [] }
                ).actions,
                function (is_assigned, action) {
                  let response = true;
                  if ("show" in action) {
                    gform.processConditions.call(
                      ...
                      },
                      {
                        _lookup: function (data, field, args) {
                          var states = _.map(data.history, function (item) {
                            return item.state;
                          });
                          states.push(
                            data.workflow.instance.configuration.initial
                          );
                          ...
                          return {
                            value: _.selectPath(stuff, "_state." + args.name),
                          };
                  ...
                  if (typeof action.assignment == "undefined") {
                    ...
                    if (action.assignment.type == "user") {
                      if (
                        gform.m(action.assignment.id, mappedData) ==
                        mappedData.actor.unique_id.toString()
                      ) {
                        return response;
                      }
                    } else if (action.assignment.type == "group") {
                      if (
                        mappedData.actor.groups.indexOf(
                          parseInt(gform.m(action.assignment.id, mappedData))
                        ) >= 0
                      ) {
                        return response;
                      }
              ...
              mappedData.resources = _.reduce(
                resources,
                function (resources, item, name) {
                  resources[name] = item;
                  return resources;
                },
                {}
              ...
              mappedData.hasFiles =
                _.filter(mappedData.history, function (item) {
                  if (item.file && item.deleted_at == null) {
                    return item;
                  }
                }).length > 0;
              ...
              this.history = new Ractive({
                el: this.container.elementOf(this).querySelector(".row .list"),
                template: templates.history,
                data: _.extend({}, this.methods, mappedData),
                partials: templates,
              });
              ...
              gform.collections.on(
                "change",
                function (e) {
                  mappedData.resources[e.collection] = gform.collections.get(
                    e.collection
                  );
                  if (typeof this.history !== "undefined") {
              ...
              $(".filterable.file").on(
                ...
                function (e) {
                  ...
                  document.querySelector(".report").innerHTML =
                    gform.renderString(workflow_report.view, this.get());
                  ...
                  file = _.find(mappedData.history, {
                  ...
                  this.ractive = new Ractive({
                    el: document.querySelector(".report .view_container"),
                    template: workflow_report.file_summary,
                    data: _.extend({}, this.methods, mappedData),
                    partials: templates,
                  });
                  $('[data-toggle="tooltip"]').tooltip();
              ...
              update = function (file, response) {
                if (typeof response !== "undefined") {
                  ...
                  if (
                    ...
                    event.user_id_created == this.get().user.id
                  ) {
                    event.user = this.get().user;
                  }
                  if (
                    event.deleted_at &&
                    typeof event.deleted_by == "undefined" &&
                    event.user_id_deleted == this.get().user.id
                  ) {
                  ...
                  newEvent.actor.is = {
                    owner:
                      mappedData.owner.unique_id == newEvent.actor.unique_id,
                  };
                  newEvent.previous = { state: event.start_state };
                  newEvent.is = {
                    open: newEvent.status == "open",
                  ...
                  if (typeof file == "object") {
                    gform.collections.update(
                      "files",
                      _.where(mappedData.history, { file: true })
                    );
                    var field = gform.instances.modal.find({
                      shown: true,
                      type: "files",
                    });
                    if (field) {
                      ...
                      $(
                        '[href="' +
                          _.find(field.options, { id: response.id }).path +
                          '"]'
                      ).click();
                    }
                  }
              ...
              $(".row .list").on(
                ...
                function (e) {
                  ...
                  var log = _.find(mappedData.history, {
                    id: parseInt(e.currentTarget.dataset.id),
                    log: true,
                  });
                    ...
                    fields: [
                      {
                        ...
                        type: "fieldset",
                        fields:
                          this.get().options.workflow_version.code.form.fields,
                      },
                      {
                        type: "hidden",
                        name: "_flowstate",
                  ...
                  // }.bind(this))
                  form.events =
                    this.get().options.workflow_version.code.form.events;
                  ...
                  if (e.currentTarget.dataset.current) {
                    ...
                    if (typeof templates.preview == "undefined") {
                      previewForm.on("change", function (e) {
                        ...
                        templates.preview =
                          '<h4>Current Data</h4><div id="previewForm">' +
                          previewForm.toString("_state") +
                          "</div>";
                      });
                      templates.preview =
                        '<h4>Current Data</h4><div id="previewForm">' +
                    ...
                    if (
                      mappedData.workflow.instance.version.code.form.files &&
                      mappedData.current_state.uploads
                    ) {
                      if (
                        $(
                          "#uploader_" +
                            mappedData.original.workflow_submission_id
                        ).length
                      ) {
                        $(
                          "#uploader_" +
                      ...
                      function (e) {
                        if (e.currentTarget.dataset.action == "delete") {
                          ...
                          myModal = new gform({
                            ...
                            data: _.find(mappedData.history, {
                              id: parseInt(e.currentTarget.dataset.id),
                            }),
                            fields: [
                    ...
                    var states = _.map(
                      ...
                      function (compareDate, item) {
                        if (
                          compareDate.isAfter(moment(item.created_at.original))
                        ) {
                          return item.state;
                        }
              ...
              $(".report").on(
                ...
                function (e) {
                  var formStructure = {
                    ...
                    id: mappedData.original.workflow_submission_id,
                    name: "modal",
                    events:
                      this.get().options.workflow_version.code.form.events ||
                      {},
                    data: {
                      user: this.get().user,
                      data: mappedData,
                      _flowstate: this.get().options.state,
                      _flowaction: e.currentTarget.dataset.event,
                      id: this.get().options.id,
                    },
                    actions: [
                      {
                  ...
                  if (
                    _.find(
                      ...
                      { name: e.currentTarget.dataset.event }
                    ).form
                  ) {
                  ...
                  new gform(formStructure)
                    .on(
                      ...
                      function (e) {
                        ...
                        var currentAction = _.find(
                          _.find(
                            this.get().options.workflow_version.code.flow,
                            { name: mappedData.state }
                          ).actions,
                          { name: e.form.get("_flowaction") }
                        );

                        if (
                          currentAction.validate !== false &&
                          !e.form.validate(true)
                        ) {
                          if (
                            currentAction.invalid_submission !== true ||
                            typeof e.form.errors.signature !== "undefined" ||
                            !confirm(
                              gform.renderString(
                            ...
                            toastr.error(
                              "Form invalid, please check for errors!",
                              "ERROR"
                            );
                            return;
                          }
                        ...
                        e.form.trigger("close");
                        formData = {
                          comment: e.form.get("comment"),
                          action: e.form.get("_flowaction"),
                        };
                        ...
                        $.ajax({
                          url: "/api/workflowsubmissions/" + e.form.get("id"),
                          dataType: "json",
                          contentType: "application/json",
                    ...
                    gform.instances.modal.fields.push(
                      gform.instances.modal.add({
                        ...
                        help:
                          _.find(
                            _.find(
                              mappedData.workflow.instance.version.code.flow,
                              { name: mappedData.state }
                            ).actions,
                            { name: e.currentTarget.dataset.event }
                          ).signature_text || "Please Sign Above",
                        name: "signature",
                        target: ".gform-footer",
                        columns: 8,
                      })
